...
namespace MerchantAPI.APIGateway.Test.Functional
{
  ...
  where TPostViewModel : class
  {
    ...
    HttpRequestMessage CreateRequestMessage(HttpMethod httpMethod, string url, HttpContent content = null)
    {
      var reqMessage = new HttpRequestMessage(httpMethod, url);
      if (!string.IsNullOrEmpty(ApiKeyAuthentication))
      {
        reqMessage.Headers.Add(ApiKeyAuthenticationHandler.ApiKeyHeaderName, ApiKeyAuthentication);
      }
    ...
    async Task<HttpResponseMessage>  PerformRequestAsync(HttpClient client, HttpMethod httpMethod, string uri, HttpContent content = null)
    {
      var reqMessage = CreateRequestMessage(httpMethod, uri, content);
    ...
    public async Task<TResponse> Get<TResponse>(HttpClient client, string uri, HttpStatusCode expectedStatusCode)
    {
    ...
    [TestMethod]
    public async Task GetCollection_NoElements_ShouldReturn200Empty()
    {

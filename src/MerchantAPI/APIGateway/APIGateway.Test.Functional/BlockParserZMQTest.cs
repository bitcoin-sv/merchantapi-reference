...
namespace MerchantAPI.APIGateway.Test.Functional
{
  ...
  public class BlockParserZMQTest : BlockParserTestBase
  {
    ...
    public async Task CatchBlockHashZMQMessage()
    {
      ...
      var (blockCount, blockHash) = await CreateAndPublishNewBlock(rpcClient, null, tx);
    ...
    public async Task QueueProcessingAfterOperationCancelled()
    {
      ...
      var (blockCount1, blockHash1) = await CreateAndPublishNewBlock(rpcClient, null, tx);
      ...
      var (_, blockHash2) = await CreateAndPublishNewBlock(rpcClient, null, tx2);
      ...
      // publish block3
      var (blockCount3, blockHash3) = await CreateAndPublishNewBlock(rpcClient, null, tx2);
      newBlockArrivedSubscription = await newBlockDiscoveredSubscription.ReadAsync(cts.Token);
